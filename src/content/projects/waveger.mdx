...
<div className="not-prose my-12">
  <CarouselStandard2
    ...
    alt={[
      'Desktop homepage with weekly chart and countdown timer',
      'Mobile homepage with weekly chart and countdown timer',
    ]}
...
<div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-12">
  <div className="order-2 lg:order-1">
    ...
    <div className="space-y-4">
      ...
      <div className="text-muted-foreground leading-relaxed">
        The Edge Function grabs the latest chart, maps predictions against
        actual positions, works out points using a tiered system (150 for an
        exact match, 100 if you're within 1 position, and so on), then fires off
        a batch SQL function. Scoring dropped from roughly 30 seconds down to
        under 2. Genuinely satisfying to watch that number fall.
      </div>
    </div>
  </div>
...
<div>
  ...
  <div className="space-y-4">
    <div className="text-muted-foreground leading-relaxed">
      Billboard chart data is rate-limited and slow (300ms+ latency per
      request). The API costs were also creeping up faster than I'd expected. To
      avoid hammering the API, chart data flows through three layers: Upstash
      Redis with a 7-day TTL, then a PostgreSQL{' '}
      <code className="rounded bg-white/10 px-1.5 py-0.5 text-sm">charts</code>{' '}
      table, then TanStack Query's client cache with a 30-minute stale time.
      Most requests never touch the external API at all.
    </div>
    <div className="text-muted-foreground leading-relaxed">
      Apple Music artwork URLs get cached separately with a 30-day TTL since
      they rarely change. On the client side, TanStack Query handles
      deduplication so navigating around the app doesn't trigger redundant
      fetches.
    </div>
  </div>
</div>
...
<div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-12">
  <div className="order-1">
    <CarouselStandard2
      images={[
        '/portfolio/waveger-03-results-reveal-desktop-thumb.webp',
        '/portfolio/waveger-03-results-reveal-mobile-thumb.webp',
      ]}
      alt={[
        'Desktop results reveal with animation and scoring breakdown',
        'Mobile results reveal with animation and scoring breakdown',
      ]}
      buttonLabels={['Desktop', 'Mobile']}
    />
  </div>
  <div className="order-2">
    ...
    <div className="space-y-4">
      ...
      <div className="text-muted-foreground leading-relaxed">
        ...
        @dnd-kit with keyboard, pointer, and touch sensors so it stays
        accessible regardless of how someone's interacting with it.
      </div>
    </div>
  </div>
</div>
